  ...
  <div class="personal_editCard">
    ...
    <div class="box">
      <van-form @submit="saveCard">
        ...
        >
          <template #extra>
            <van-checkbox v-model="checked" @click="today" shape="square" checked-color="#f8d57e">至今</van-checkbox>
          </template>
        </van-field>
        <!-- <van-calendar
        ...
        <van-field v-model="cardInfo.industry" label="行业" placeholder="请输入中文名(必填)" name="industry" />
        <van-field
...
export default {
  ...
  methods: {
    ...
    saveCard(cardInfo) {
      ...
      this.checked == true
        ? (cardInfo.endDate = "")
        : (cardInfo.endDate = cardInfo.endDate + "-" + "01");
