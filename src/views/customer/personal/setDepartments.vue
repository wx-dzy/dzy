<template>
  ...
  <div class="setDepartments">
    ...
    <div class="item">
      ...
      >
        <div class="left">
          <img :src="item.avatar" @error="avatarErr(item)" alt />
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      sysName: this.$route.query.sysOrganizationName,
    };
  },
  created() {
    this.getChildren();
  ...
  methods: {
    ...
    getChildren() {
      ...
      Api.getChildDeptandUser(this.sysOrganizationId).then((res) => {
        // console.log("获取子部门及其人员:", res);
        // console.log("名：" + this.sysOrganizationName);
        let { code, data, msg, total } = res;
        if (code == 200) {
